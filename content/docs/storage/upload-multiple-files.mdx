...
<Callout title="Origin UI Integration" type="note">
  This component uses the **multiple file upload with progress track** design from [Origin UI](https://originui.com/file-upload) and integrates it with the `generatePresignedUploadUrl` API to provide real progress tracking and actual file uploads to cloud storage buckets.
</Callout>
